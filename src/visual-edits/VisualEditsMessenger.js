...
export default function HoverReceiver() {
  ...
  const originalSrcRef = useRef(""); // track original img src
  const focusedImageElementRef = useRef(null); // track the actual img element
  ...
  useEffect(() => {
    if (isVisualEditMode) {
      ...
      }, 500); // Wait 500ms for DOM is fully ready
    }
  }, []); // Run only on mount
  ...
  useEffect(() => {
    ...
    function onMsg(e) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES") {
        const allMatchingElements = document.querySelectorAll(
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER") {
